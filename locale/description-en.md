...
**TL;DR:** Version 0.1.1 groups shortcuts by purpose, improves key-text alignment and verifies key capture. Grid retains number keys for groups: Ctrl+number assigns, number selects, press again to focus. Activate the module, open with F12, edit and Apply.
...
Activate the module in UCP and start the game: it is active immediately, with no additional launcher switch or customization options. Open **Custom Hotkeys** on the main menu or press **F12** in a supported menu or live single-player game. **Ctrl+Shift+F12** opens the recovery entry if F12 was rebound. Click an action row, or select it and press **Enter**, then press the combination. **Delete** clears the selected binding; **Apply** saves changes. The editor follows the **game language**; this description follows the launcher language.
