...
local M={}
local installed
function M.install(cursor,router,camera,quickslot,lowering,navigation)
  ...
  result.callback=ffi.cast('void (__cdecl *)(void *)',function(mouse)
    if (not cursor.pending and camera.count==0 and not (quickslot and quickslot.active)
        and not (lowering and lowering.active) and not (navigation and navigation.pending)) or mouse~=nativeMouse then return end
