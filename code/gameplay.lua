...
function M.resolve(s,ownedModal)
  ...
  if (ownedModal==5 and not options) or (ownedModal==9 and not load) then return nil end
  if (s.screen~=14 and s.screen~=16) or s.mode==1
      ...
      or (s.paused~=0 and not ((options or load) and s.paused==1))
      or s.halted~=0 or s.sliding~=0 or (s.modal~=-1 and (ownedModal==nil or s.modal~=ownedModal))
      or s.modal2~=-1 or s.modal3~=-1
      or (s.textModal~=0 and not options and not load) or s.textEditor~=0 or s.newPlayer~=0 or s.delay~=-1
